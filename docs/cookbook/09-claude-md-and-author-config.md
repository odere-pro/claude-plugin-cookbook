...
> **Intent:** Draw the line between what a plugin _ships_ and the _author-only_ config that turns a
...
where things live, the gate map, where to read before editing. It is project memory for _this repo_,
...
> cache. Author-only files cost nothing at runtime because Claude only _loads_ recognized component
